...
1. 从 `src/content/research/hec.md` 复制一份 Markdown，使用稳定、唯一的 kebab-case 文件名作为研究 slug。成果页地址为 `/dataset/<slug>/`，如 `/dataset/hec/`；内容文件仍放在原目录，`year` 仅用于展示、排序与旧地址兼容。
...
旧的 `/<year>/<slug>/` 地址通过静态兼容页跳转到 `/dataset/<slug>/`，启用 JavaScript 时保留查询参数和章节锚点；禁用 JavaScript 时仍可跳转到新页面。修改已发布成果的 `year` 时应保留原年份地址的跳转，新地址不受年份变化影响。
...
页面允许带或不带末尾斜杠访问；站内页面链接和静态构建的 canonical 统一使用带斜杠地址，如 `/dataset/hec/`。
